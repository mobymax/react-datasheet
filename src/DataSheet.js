...
import {
  ...
  LEFT_KEY, UP_KEY, DOWN_KEY, RIGHT_KEY, C_KEY, V_KEY,
} from './keys';
...
export default class DataSheet extends PureComponent {
  ...
  handlePaste(mixed) {
    if (isEmpty(this.state.editing)) {
      ...
      const {
        data, onCellsChanged, onPaste, onChange,
      } = this.props;
      if (onCellsChanged) {
        const additions = [];
        pasteData.forEach((row, i) => {
          row.forEach((value, j) => {
            ...
            if (!cell) {
              ...
              changes.push({
                cell, row: end.i, col: end.j, value,
              });
            }
          });
        });
  ...
  onMouseDown(i, j) {
    const editing = (isEmpty(this.state.editing) || this.state.editing.i !== i || this.state.editing.j !== j)
      ? {} : this.state.editing;
    this._setState({
      selecting: true, start: { i, j }, end: { i, j }, editing, forceEdit: false,
    });
  ...
  onChange(row, col, value) {
    ...
    if (onCellsChanged) {
      onCellsChanged([{
        cell: data[row][col], row, col, value,
      }]);
    } else if (onChange) {
  ...
  render() {
    const {
      ...
      className, overflow, data, keyFn, mobile,
    } = this.props;
